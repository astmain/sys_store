...
export async function canvas_three_parse({ canvas, file }: { canvas: any, file: any }) {
  ...
  // 渲染器
  let renderer = new THREE.WebGLRenderer({ canvas, antialias: true }) //antialias是否执行抗锯齿。默认为false
  //@ts-ignore
  ...
  // 材料-外观
  const my_material: THREE.MeshStandardMaterial = new THREE.MeshStandardMaterial({ color: 0xbabcbd })

  let blobURL = URL.createObjectURL(file) //得到blobURL
  // 结构-几何
  let my_geometry: THREE.BufferGeometry<THREE.NormalBufferAttributes> = await parse_STLLoader_by_blobURL(blobURL) as THREE.BufferGeometry<THREE.NormalBufferAttributes>
  if (!my_geometry) {
    throw new Error("my_geometry is null")
  }
  if (!(my_geometry instanceof THREE.BufferGeometry)) {
    throw new Error("my_geometry is not a BufferGeometry")
  }

  // 动态-自动居中 & 缩放场景
  my_geometry.computeBoundingBox()
  const boundingBox: THREE.Box3 | null = my_geometry.boundingBox
  if (!boundingBox) {
